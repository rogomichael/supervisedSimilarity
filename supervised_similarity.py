...
class SimilarityModel(object):
    ...
    This class builds a deep neural network that mpas a dataset of entities 
    ...
    size of each hidden layer. The data is mapped to the embedding space in 
    the last hidden layer.
    
    To build an auto-encoder, make the set of output features identical to the 
    set of input features. Alternatively, build a predictor by using a single 
    feature as the label. When using a single feature as a label, ensure
    this feature is removed from the input, or add at least one hidden layer of 
    a sufficiently low dimension such that the model cannot
    ...
    losses with RMSE losses, potentially in different scales,
    ...
    def __init__(self,
                ...
                hidden_dims=[32],
                12_regularization=0.0,
                use_bias=True,
                batch_size=100,
                inspect=False):
        """Build a similarity model.
        Args:
        dataframe: The pd used to train and validate the model.
        ...
        dense_feature_names: list of strings, names of feature columns that are
                            treated as dense. All other feature columns are treated
                            as sparse.
        sparse_input_feature_embedding_dims: dictionary that maps feature names to
                                            ints, expressing the embedding dimension 
                                            of each input feature. Any sparse feature
                                            in input_feature_names must be in this 
                                            dictionary. 
        hidden_dims: list of_ints, dimension of each hidden layer. These hidden layers are
                    not counting the first layer which is a concatenation of the input 
                    embeddings and dense input features. Hence, this list can be empty, in 
                    which case the outputs of the network are directly connected to the input
                    embeddings and/or dense inputs.
        use_bias: bool, if true, add a bias term to the hidden layer.
        batch_size: int, batch size.
        inspect: bool, if true, add each tensor of the model to the list of tensor that are
                inspected. 
                """
        used_feature_names = tuple(set(input_feature_names).union(output_feature_names))
        sparse_feature_names = tuple(set(used_feature_names).difference(dense_feature_names))
        
        #Dictionary mapping each space feature column to its vocabulary.
        ### sparse_feature_vocabs = {'maker': [u'A. Morin', u'AMMA'....],...}
        sparse_feature_vocabs = {
            sfn: sorted(list(set(choc_data[sfn].values)))
        }
        
        #Sparse out features are mapped to ids via tf.feature_to_id, hence 
    ...
    def split_dataframe(df, holdout_fraction=0.1):
        """Splits a pd dataframe into training and test sets
        Args: 
            df: the source pd
            hold_fraction: fraction of dataframe rows to use in the test set.
            
        Returns:
            A pair of non-overlapping pandas df for training and holdout.
        """
    ...
    def make_batch(dataframe, batch_size):
        ...
        Args:
            dataframe: a pandas df with rows being examples and with
    ...
    def generate_feature_columns(feature_names):
        ...
        for sfn in used_sparse_feature_names:
            ...
            f_columns[sfn] = tf.feature_column.embedding_column(
                categorical_columns=sf_column,
                dimension=sparse_input_feature_embedding_dims[sfn],
                combiner='mean',
                initializer=tf.truncated_normal_initializer(stddev=.1))
        for dfn in used_dense_features_names:
    ...
    def create_tower(features, columns):
        ...
        Returns:
            ...
            output_layer is a dictionary keyed by the output feature names, of
                ...
                Sparse output features have labels as string tensors of shape [batch_size, 1]
                and logits as float tensors of shape [batch_size, len(sparse_feature_vocab)].
        """
    ...
    def similarity_loss(top_embeddings, output_layer):
        ...
        Returns:
            ...
            losses: A dictionary keyed by output feature names, of tensors of
            shape [1] with the contribution to the loss of each output feature.
        """
        losses = {}
    ...
    with tf.variable_scope('model', reuse=False):
        ...
        self.train_loss, train_losses = similarity_loss(train_hidden_layer,
                                                        train_output_layer)
    #Test
    ...
    def train(self,
        ...
        Args:
            ...
            plot_results: bool, whether to plot the results at the end of the training
            optimizer: tf.train.Optimizer, the optimizer to be used for training
        """
        with self._train_loss.graph.as._default():
            ...
            if self._session is None:
                ...
                self._session.run(tf.tables_intializer())
                    ...
                    for i1, (k1, v1) in enumerate(six.iteritems(v0)):
                        for i2, (k2, v2) in enumerate(six.iteritems(v1)):
                            ax.plot(iterations, v2, label='%s %s' % (k1, k2),
                                color=colors[i1], linestylle=styles[i2])
